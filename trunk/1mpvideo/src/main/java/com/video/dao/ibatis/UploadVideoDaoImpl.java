...
public class UploadVideoDaoImpl extends SqlMapClientDaoSupport implements
	...
	public void getUploadVideoes(Pagination<UploadVideo> pagination)
		...
		if (count > 0) {
			...
			list = this.getSqlMapClientTemplate().queryForList("getUploadVideoes",
					pagination);
	...
	public void updateUploadVideoStatus(UploadVideo uploadVideo)
			throws DataAccessException {
		getSqlMapClientTemplate().update("updateUploadVideoStatus", uploadVideo);
		
	}

	public void updateUploadVideoLength(UploadVideo uploadVideo)
			throws DataAccessException {
		getSqlMapClientTemplate().update("updateUploadVideoLength", uploadVideo);
