...
function toPage(){
	...
	var val = obj.options[index].value;
	 window.location.href="getTags.do?pageNo="+val; 
}
...
function lock(id,pageNo){
	document.form.action="updateTag.do?id="+id+"&pageNo="+pageNo+"&type=lockOne";
	var i = confirm("确定锁定?");
	   if(i){
...
function del(id,pageNo){
	...
	document.form.action="updateTag.do?id="+id+"&pageNo="+pageNo+"&type=deleteOne";
	var i = confirm("确定删除?");
	   if(i){
	   document.form.submit();
	   };
...
function doit(pageNo){
	document.form.action="updateTag.do?pageNo="+pageNo+"&type=all";
	var i = confirm("确定执行?");
	   if(i){
	   document.form.submit();
	   };
...
function createTag(){
	document.form.action="createTag.do";
	var i = confirm("确定添加?");
	   if(i){
	   document.form.submit();
	   };
...
<table width="701" border="0" id="mytable">
  ...
  <tr>
    <th width="51"><div align="center">选中</div></th>
    <th width="58"><div align="center">ID</div></th>
    <th width="103"><div align="center">标签</div></th>
    <th width="94"><div align="center">关联视频</div></th>
    <th width="118"><div align="center">添加时间</div></th>
    <th width="88"><div align="center">状态</div></th>
    <th width="148"><div align="center">操作</div></th>
  </tr>
<c:forEach items="${pagination.results}" var="result" varStatus="stat">
  <tr ${stat.count%2==0? "bgcolor='#F1D1FA'":""}>
    ...
    <td><div align="center">${fn:length(result.videoes)} </div></td>
