...
function infoTimeout() {
	...
	function savedata(){
		...
		var url = 'updateIndexRecommendVideo.do';
...
<c:forEach items="${recommendVideoes}" var="video">
  <tr>
    <td width="58">${video.id }</td>
    <td width="88">${video.name}</td>
    <td width="118">${video.video.id }</td>
<td width="100"><img width="100" src="<%=application.getAttribute("IMAGE_URL")%>/${video.imageUrl }"/></td>
    <td width="148"><a href="javascript:void(0)" onclick="removeRow(this)">删除</a> <a href="getIndexRecommendVideo.do?id=${video.id }">修改</a></td>
