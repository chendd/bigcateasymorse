...
<form action="updateVideoChannel.do" method="post" name="videoChannelForm">
	<input type="hidden" name="flag" value="1" />
	<input type="hidden" name="id" value="${videoChannel.id }" />
				...
				<label>
					<select name="parentId">
						...
						<c:forEach items="${videoChannelList}" var="videoChannels">
							<c:choose>
								<c:when test="${videoChannels.id==videoChannel.parent.id}">
									<c:set value="selected" var="selectVideoChannel" />
								</c:when>
								<c:otherwise>
