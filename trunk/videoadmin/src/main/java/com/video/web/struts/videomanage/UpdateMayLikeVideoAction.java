...
public class UpdateMayLikeVideoAction extends BaseAction{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if("ajax".equals(type)){
			...
			List<MayLikeVideo> list = new ArrayList<MayLikeVideo>();
			Long i=new Long(1);
				...
				for(String strId : ids){
					...
					if(mayLikeVideo!=null){
						mayLikeVideo.setId(i);
						
						
					}
					else{
						mayLikeVideo = new MayLikeVideo();
						mayLikeVideo.setId(i);
						Long vid = Long.parseLong(vids[i.intValue()-1]);
						
						Video video = getAllDao().getVideoById(vid);
						mayLikeVideo.setName(video.getName());
						mayLikeVideo.setDescription(video.getDescription());
						mayLikeVideo.setImageUrl(video.getImage());
						mayLikeVideo.setVideo(video);
					}
					list.add(mayLikeVideo);
					i++;
				}
				getAllDao().deleteMayLikeVideo();
				for(MayLikeVideo video:list){
					getAllDao().createMayLikeVideo(video);
			...
			try {
				...
				if (!myFile.isMissing()) {
					...
					String newFileName = SomeStatic.UPLOAD_USERFACE_URL+"/upload/videoImg/maylikeVideo"+id+"." + FileType; // 可自动生成文件名以防止同名覆盖
					myFile.saveAs(newFileName);
					
				      mayLikeVideo.setImageUrl("/upload/videoImg/maylikeVideo"+id+"."+FileType);
				     getAllDao().updateMayLikeVideo(mayLikeVideo);
				}
			} catch (Exception e) {
			...
			if(strId!=null && !strId.trim().isEmpty()){
				...
				MayLikeVideo mayLikeVideo = getAllDao().getMayLikeVideoById(id);
				mayLikeVideo.setDescription(description);
				mayLikeVideo.setName(name);
				getAllDao().updateMayLikeVideo(mayLikeVideo);
